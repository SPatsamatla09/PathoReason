"""Run PathoReason predictors over the PCam test set.

Week 3 PCam inference runner.

Features
--------
- Streams the official PCam test partition.
...
CACHE_ROOT = REPO_ROOT / "results" / "pcam" / "inference_cache"
OUTPUT_ROOT = REPO_ROOT / "results" / "pcam" / "inference"
...
def run(args: argparse.Namespace) -> None:
    ...
    total = args.limit
    print(
        "PCam test samples selected: "
        + (str(total) if total is not None else "full test set")
    )
    ...
    for index, sample in enumerate(dataset):
        ...
        try:
            ...
            # Translate PLIP's internal MHIST class names in PCam metadata.
            if args.model == "plip":
                ...
                if probabilities is not None:
                    metadata["class_probabilities"] = {
                        "normal": probabilities["HP"],
                        "tumor": probabilities["SSA"],
                    }
            ...
            record = {
                ...
                "predicted_label": (
                    {"HP": "normal", "SSA": "tumor"}[prediction.label]
                    if args.model == "plip"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run PathoReason inference on PCam."
    )
