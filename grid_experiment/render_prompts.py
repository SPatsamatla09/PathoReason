...
at call time. Run from grid_experiment/:
    ...
    python3 render_prompts.py            # write prompts/rendered/*.txt
...
OUT = os.path.join(ROOT, "prompts", "rendered")
            ...
            index.append(
                {
                    ...
                    "file": f"prompts/rendered/{pid}.txt",
                }
            )
    with open(os.path.join(OUT, "index.json"), "w") as fh:
        json.dump(index, fh, indent=2)
    print(f"wrote {len(index)} prompts to prompts/rendered/")
