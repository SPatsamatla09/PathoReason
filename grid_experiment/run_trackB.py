"""Run the pathoreason tiles through Track B (Sid's GPT4Predictor interface).
...
Output goes to runs/<prompt>_trackB.jsonl -- the gemma runs are never touched.
...
# Same repeated tiles as the gemma runs, for the same stability check.
...
def build_predictor(repo):
    ...
    return cls(model=TRACK_B_MODEL, image_detail=IMAGE_DETAIL)
