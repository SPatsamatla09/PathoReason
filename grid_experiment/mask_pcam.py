...
re-overlay.
    ...
    python3 mask_pcam.py --image pcam_test_000000.png --cells B2,B3,C2,C3
    python3 mask_pcam.py --from-run pcam/runs/cte_p1_trackB.jsonl
"""
...
CLEAN = os.path.join(ROOT, "pcam", "clean")
OUT = os.path.join(ROOT, "pcam", "masked")
SEED = 20260806
...
def build(image_name, cited, rng, regrid=True, out_root=OUT, subset=None):
    ...
    if not os.path.exists(src):
        raise FileNotFoundError(f"{src} -- mask the clean tile; run select_and_render_pcam.py first")
    rgb = load_rgb(src)
...
def main():
    ...
    ap.add_argument("--image", help="tile filename, e.g. pcam_test_000000.png")
