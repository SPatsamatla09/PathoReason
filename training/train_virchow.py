"""Train and evaluate a Virchow linear probe on MHIST."""
...
from models.virchow_baseline import VirchowEncoder
...
CACHE_DIR = Path("results") / "embedding_cache" / "virchow"
TRAIN_CACHE = CACHE_DIR / "mhist_train.pt"
TEST_CACHE = CACHE_DIR / "mhist_test.pt"

CHECKPOINT_PATH = (
    Path("models") / f"A_mhist_seed{SEED}" / "virchow.pt"
)


def main() -> None:
    """Run the frozen-Virchow MHIST linear-probe experiment."""
    ...
    if TRAIN_CACHE.is_file() and TEST_CACHE.is_file():
        print("Loading cached Virchow embeddings...")

        train_embeddings, train_labels = load_embedding_cache(
            TRAIN_CACHE
        )
        test_embeddings, test_labels = load_embedding_cache(
            TEST_CACHE
        )

    else:
        print("Loading Virchow encoder...")
        encoder = VirchowEncoder()

        print("Building MHIST loaders with Virchow preprocessing...")
        ...
        save_embedding_cache(
            ...
            metadata={
                ...
                "split": "train",
                "seed": SEED,
                "embedding_dim": VirchowEncoder.embedding_dim,
            },
        )
    ...
    if train_embeddings.shape[1] != VirchowEncoder.embedding_dim:
        raise RuntimeError(
            "Unexpected Virchow embedding dimension: "
    ...
    output_path = Path(RESULTS_DIR) / "baselines" / "virchow.json"
